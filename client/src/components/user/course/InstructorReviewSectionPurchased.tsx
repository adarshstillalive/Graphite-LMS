...
const InstructorReviewSectionPurchased = ({
  instructorId,
  userId,
}: {
  instructorId: string;
  ...
  const [userReview, setUserReview] = useState<IInstructorReviewPopulated>();
  ...
  const loadReviews = async () => {
    try {
      const response = await fetchInstructorReviewsWithUser(
        userId,
        instructorId,
        currentPage
      );
  ...
  const handleAddReview = async () => {
    ...
    try {
      ...
      await addInstructorReview(newReview);
      setReviewText('');
      setCurrentRating(0);
      setReviews([]);
      setCurrentPage(1);
  ...
  const handleUpdateReview = async () => {
    try {
      ...
      await addInstructorReview(updatedReview);
      setIsEditing(false);
      setReviews([]);
      setCurrentPage(1);
      toast({
...
export default InstructorReviewSectionPurchased;
