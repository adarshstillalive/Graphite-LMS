...
import { fetchReviews } from '@/services/user/courseService';
import { Button } from '@/components/ui/button';

const ReviewSection = ({ courseId }: { courseId: string }) => {
  ...
  const loadReviews = async () => {
    try {
      const response = await fetchReviews(courseId, currentPage);
      const result = response.data;

      setReviews((prev) => {
        const existingIds = new Set(prev.map((r) => r._id));
        const filteredNewReviews = result.data.filter(
          (r: IReviewPopulated) => !existingIds.has(r._id)
        );
