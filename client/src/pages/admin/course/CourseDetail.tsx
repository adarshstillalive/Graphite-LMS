...
const CourseDetail = () => {
  ...
  const handleApproveCourse = async () => {
    try {
      const response = await approveCourseRequest(course._id);
      if (response.success) {
  ...
  const handleRejectCourse = async () => {
    try {
      const response = await rejectCourseRequest(course._id, rejectionReason);
      if (response.success) {
        navigate('/admin/courses');
      }
    } catch (error) {
        ...
        <header className="mb-8">
          ...
          <div className="flex items-center mb-4">
            <span className="text-gray-600">
              ...
              {course.category.subCategory.reduce(
                (acc: ICategory, curr: ICategory) => {
          ...
          {(!course.isApproved || course.isRejected) && (
            <aside className="md:w-1/3">
              <div className="bg-white flex flex-col gap-2 rounded-lg border-4 border-gray-600 p-6 sticky top-8">
                <h2 className="text-2xl text-center font-bold mb-4">Actions</h2>
                <Button onClick={handleApproveCourse}>Approve Course</Button>
                {!course.isRejected && (
